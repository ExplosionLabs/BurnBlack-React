...
export const fetchDividendData = async (token: string) => {
  ...
  export const fetchAgriAssestData = async (token: string) => {
    try {
      console.log("ad");
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/v1/fillDetail/getagri-income`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  export const fetchBussinessFundData = async (token: string, type: string) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/v1/fillDetail/getBussiness-fund/${type}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
